...
void GenerateRookAndQueenMoves(const int us, const Position *pos, MoveList *moveList, const uint64_t targetBitboard, uint64_t occupied, int typeOfMove)
{
    uint64_t ourRooks = (pos->type_of_pieces[ROOK]|pos->type_of_pieces[QUEEN]) & pos->occupancy_by_color[us];
